...
$lstUltimosKmCargados = $Porterias->ultimoKmCargado();

if (isset($_POST['submit'])) {

  foreach ($lstUltimosKmCargados as $row) {
    if (isset($_POST['km'.$row['id']])) {
      $km = $_POST['km'.$row['id']];
      if ($km >= $row['kilometros']) {
        $Camiones->buscarPorId($row['id']);
        $Camiones->modificarFilter(['kilometros'=>$km]);
      }
        ...
        <div class="card">
          ...
          <div class="card-body px-2 pb-4">
            <div class="table-responsive">
                ...
                <button type="button" class="btn bg-gradient-info btnVolver">
                VOLVER
                </button>
                ...
                <thead>
                  <tr>
                    ...
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Km</th>
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Ultimos Km cargados</th>
                  </tr>
                </thead>
                <tbody>
                  <?php
                  ...
                  <tr>
                    ...
                    <td><input class="form-control" type="number" name="km<?php echo $row['id']; ?>" id="km<?php echo $row['id']; ?>" value="<?php echo $row['km']; ?>"/></td>
                  </tr>
                  <?php
                  }
                  ?>
                  <tfoot>
                    <tr>
                      <td colspan="3">
                      <button type="submit" name="submit" class="btn bg-gradient-success btnGuardar">
                      GUARDAR TODO
                      </button>
                      </td>
                    </tr>
