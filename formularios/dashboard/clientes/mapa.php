...
if (isset($_GET['idcliente'])) {
  if ($_GET['idcliente']== 0) {
    $resClientes = $Clientes->traerTodos();
  } else {
    $resClientes = $Clientes->traerTodosFilter(['id'=>$_GET['idcliente']]);
  }
                ...
                <div class="card">
                    ...
                    <div class="card-body px-2 pb-4">
                      <div class="row">
                        <div class="col-8">
                          ...
                          <div class="form-group">
                            <select class="form-control" name="idcliente" id="idcliente">
                              ...
                              <?php foreach ($Clientes->traerTodos() as $row) { ?>
                        ...
                        <div class="col-4">
                          <label for="refsemaforo_filtro" class="control-label">Acciones</label>
                          <div class="form-group">
    ...
    <script>
      ...
      function initMap() {
        ...
        ?>
            var locations = [
                <?php 
                  foreach ($resClientes as $row) { 
                ?>
                ['<h4><?php echo $row['nombre']; ?></h4><p><?php echo $row['direccion']; ?></p><p><?php echo $row['telefono']; ?></p><p><?php echo $row['email']; ?></p><h5><a href="sucursalesmapa.php?id=<?php echo $row['id']; ?>">Ver Sucursales</a></h5>', <?php echo $row['latitud']; ?>, <?php echo $row['longitud']; ?>, <?php echo $row['id']; ?>],
                
                <?php 
                  $Sucursales = new Sucursales(1,$row['id']);
                  $res = $Sucursales->traerTodosFilter(array('reftabla'=>1,'idreferencia'=>$row['id']));
                  
                  foreach ($res as $rowS) { 
                 
                ?>
                ['<h4><?php echo $row['nombre']; ?> - <?php echo $rowS['sucursal']; ?></h4><p><?php echo $rowS['direccion']; ?></p><p><?php echo $rowS['telefono']; ?></p><h5>Sucursal</h5>', <?php echo $rowS['latitud']; ?>, <?php echo $rowS['longitud']; ?>, <?php echo $rowS['id']; ?>],
                <?php 
                  }
                }
                ?>
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $('#filtrar').click(function(){
              $(location).attr('href','mapa.php?idcliente=' + $('#idcliente').val());
            });

            $('#idcliente').val(<?php echo $idcliente; ?>);
